...
describe("Test Fixtures", () => {
  ...
  describe("Fixture Coverage", () => {
    test("should cover all output types", () => {
      ...
      expect(validFixtures.some((f) => f.includes("comment") || f.includes("simple"))).toBe(true);
      expect(validFixtures.some((f) => f.includes("label") || f.includes("pr-labels"))).toBe(true);
      expect(
        validFixtures.some((f) => f.includes("create-issue") || f.includes("multiple-outputs")),
      ).toBe(true);
      expect(validFixtures.some((f) => f.includes("discussion"))).toBe(true);
      expect(validFixtures.some((f) => f.includes("pr") || f.includes("create-pr"))).toBe(true);
    ...
    test("should cover all trigger types", () => {
      ...
      expect(validFixtures.some((f) => f.includes("simple") || f.includes("complex"))).toBe(true); // Issue triggers
    ...
    test("should cover validation error cases", () => {
      ...
      expect(
        invalidFixtures.some((f) => f.includes("missing-name") || f.includes("empty-name")),
      ).toBe(true);
    });
  });
});
