...
import { handler } from "./reopen-issue";

describe("ReopenIssueHandler", () => {
  ...
  describe("handler properties", () => {
    it("should have correct name", () => {
      expect(handler.name).toBe("reopen-issue");
    });
  });
  ...
  describe("generateSkill", () => {
    ...
    it("should include JSON schema with all fields", () => {
      ...
      expect(skill).toContain('"comment": "string"');
    });
    ...
    it("should show unlimited constraint when no max specified", () => {
      ...
      expect(skill).toContain("Maximum reopens: unlimited");
    });
    ...
    it("should show max constraint when specified", () => {
      ...
      expect(skill).toContain("Maximum reopens: 5");
    });

    it("should include constraint about closed state", () => {
      const config = {};
      const skill = handler.generateSkill(config);

      expect(skill).toContain("Issue/PR must be in closed state");
    });
    ...
    it("should include example", () => {
      ...
      expect(skill).toContain('"comment":');
      expect(skill).toContain("Reopening");
    });
    ...
    it("should include multiple file naming pattern", () => {
      ...
      expect(skill).toContain("reopen-issue-1.json");
      expect(skill).toContain("reopen-issue-2.json");
    });
    ...
    it("should include important notes about Write tool", () => {
      ...
      expect(skill).toContain("reopened immediately");
    });
  });
  ...
  describe("generateValidationScript", () => {
    it("should generate validation script", () => {
      ...
      expect(script).toContain('find /tmp/outputs -name "reopen-issue*.json"');
    ...
    it("should reopen issue via GitHub API with PATCH", () => {
      const config = {};
      const script = handler.generateValidationScript(config, mockRuntime);
      ...
      expect(script).toContain('-f state="open"');
    });

    it("should add optional comment if provided", () => {
      const config = {};
      const script = handler.generateValidationScript(config, mockRuntime);

      expect(script).toContain('if [ -n "$COMMENT" ]');
      expect(script).toContain("/issues/$ISSUE_NUMBER/comments");
      expect(script).toContain('-f body="$COMMENT"');
    });
