...
- Optional: `permissions`, `outputs`, `inputs`, `claude`, `allowed-users`, `allowed-paths`, `triggerLabels`, `rateLimitMinutes`
- Markdown body: Natural language instructions for Claude

### Input Collection System

The input system ([src/generator/input-collector.ts](src/generator/input-collector.ts)) enables agents to collect repository data before execution:
...
1. `pre-flight` job runs `collect-inputs` step that:
   - Queries GitHub API using `gh` CLI
   - Filters and formats data based on configuration
   - Outputs `has-inputs=true/false` and `inputs-data`
2. `claude-agent` job conditionally runs if `has-inputs=true`
3. Collected data is injected into context file before Claude execution

**Implementation Details**:
- Input collection script generated by `InputCollector.generateCollectionScript()`
...
- Data validated against schemas in [src/parser/schemas.ts](src/parser/schemas.ts)
